package com.nec.zeusas.security.auth.utils;
...
public class VerifyCode {
	 ...
	 * @return 图片缓存对象
	 */
	public static BufferedImage getImageCode(String textCode,//
			int width,//
			int height, //
			int interLine,//
		...
		// 画背景图
		graphics.setColor(null == backColor ? rndColor() : backColor);
		...
		if (interLine > 0) {
			...
			for (int i = 0; i < interLine; i++) {
				graphics.setColor(null == lineColor ? rndColor()
						: lineColor);
		...
		for (int i = 0; i < textCode.length(); i++) {
			fy = randomLocation ? (int) ((Math.random() * 0.3 + 0.6) * height)
					: fy;
			graphics.setColor(null == foreColor ? rndColor()
					: foreColor);
			// 将验证码字符显示到图象中
